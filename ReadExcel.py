...
def getdata(year,provincename,type = ""):
    if(type == "县级市"):
        df = pd.read_excel(r"data\县级市.xlsx",index_col=0,header=1)	# 即指定第一列为行索引,第二行为列索引
    elif(type == "地级市" or type == ""):
        df = pd.read_excel(r"data\地级市.xlsx",index_col=0,header=1)	# 即指定第一列为行索引,第二行为列索引
    elif(type == "市辖区"):
        df = pd.read_excel(r"data\市辖区.xlsx",index_col=0,header=1)	# 即指定第一列为行索引,第二行为列索引
        ...
        if(data[index]=='[' or data[index]=='［'):
            ...
            if(first<second and second<last):
                ...
                if index in Dict: 
                    ...
                    #Dict[key]+=val
                else: Dict.setdefault(other,[]).append(Tuple)
